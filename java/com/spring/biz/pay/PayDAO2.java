...
public class PayDAO2 {
	...
	// 결제 목록 출력
		...
		private static final String SQL_SELECTALL_ADMIN=
				"SELECT PAY.PAYNUM,(SELECT PRODUCT.PRODUCTNAME FROM PAYDETAIL INNER JOIN PRODUCT ON PRODUCT.PRODUCTNUM = PAYDETAIL.PRODUCTNUM WHERE PAY.PAYNUM = PAYDETAIL.PAYNUM AND ROWNUM = 1) PRODUCTNAME,(SELECT COUNT(PAYDETAIL.PRODUCTNUM)-1 FROM PAYDETAIL WHERE PAY.PAYNUM=PAYDETAIL.PAYNUM) PAYCOUNT,MEMBERID,PAYTIME,PAYMETHOD,ZIPCODE,ADDRESS,ADDRESSDETAIL,STATUS FROM PAY ORDER BY PAY.PAYNUM DESC";
